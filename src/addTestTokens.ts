...
async function requestTestnetFaucet(address: string) {
  try {
    ...
    if (response.status === 429) {
      ...
      console.log(
        `Faucet is rate limited. Try again after ${newTime.toLocaleString()}`
      );
      return;
    }
...
async function addLocalTokens(reciever: string) {
  ...
  const result = await wallet.signAndSubmitTransferTx({
    ...
    destinations: [
      {
        address: reciever,
        attoAlphAmount: alephium.utils.convertAlphAmountWithDecimals(6)!,
      },
    ],
  });

  console.log(
    `Faucet Request Successful: ${EXPLORER_URL}/transactions/${result.txId}`
  );
}
